...
namespace OpenLiveWriter.ApplicationFramework
{
    ...
    public class CommandContextMenu : ContextMenu
    {
        ...
        /// <param name="commandContextMenuDefinition">
        /// The CommandContextMenuDefinition of the commands to show in the menu.
        /// These commands must be present in the CommandManager.
        /// </param>
        /// <returns>The command that was selected; or null if no command was selected.</returns>
        public static Command ShowModal(
            CommandManager commandManager,
            Control parentWindow,
            Point position,
            CommandContextMenuDefinition commandContextMenuDefinition) =>
            ShowModal(commandManager, parentWindow, position, position.X, commandContextMenuDefinition);
            ...
            CommandContextMenuDefinition commandContextMenuDefinition)
        {
            ...
            var parentContextMenu = parentWindow.ContextMenu;

            //	Instantiate the CommandContextMenu from the command context menu definition.
            var commandContextMenu = new CommandContextMenu(commandManager, commandContextMenuDefinition);
            ...
            var command = commandContextMenu.ShowModal(parentWindow, position, alternateXPosition);
        ...
        public static Command ShowModal(Control parentWindow, Point position, CommandCollection commandCollection, bool commandBar) =>
            ShowModal(parentWindow, position, position.X, commandCollection, commandBar);
        ...
        public static Command ShowModal(Control parentWindow, Point position, int alternateXPosition, CommandCollection commandCollection, bool commandBar)
        {
            ...
            var commandContextMenu = new CommandContextMenu(commandCollection, commandBar);
        ...
        public static Command ShowModal(Control parentWindow, Point position, int alternateXPosition, MenuItem[] menuItems)
        {
            ...
            //	Instantiate the CommandContextMenu from the command collection.
            var commandContextMenu = new CommandContextMenu(menuItems);
        ...
        public static Type ShowModal(CommandManager commandManager, Control parentWindow, Point position, Type[] commandTypes)
        {
            ...
            var commandSelected = ShowModal(parentWindow, position, commandCollection, false);

            //	Remove the commands from the system command manager.
            commandManager.Remove(commandCollection);

            //	Done!
            return commandSelected?.GetType();
        }
        ...
        private CommandContextMenu(CommandManager commandManager, CommandContextMenuDefinition commandContextMenuDefinition) =>
            this.BuildMenuItems(commandManager, commandContextMenuDefinition);
        ...
        private CommandContextMenu(CommandCollection commandCollection, bool commandBar)
        {
            ...
            this.BuildMenuItems(commandCollection);
        }

        private CommandContextMenu(MenuItem[] menuItems) => this.MenuItems.AddRange(menuItems);
        ...
        private void BuildMenuItems(CommandManager commandManager, CommandContextMenuDefinition commandContextMenuDefinition)
        {
            ...
            {
                this.MenuItems.AddRange(MenuBuilder.CreateMenuItems(commandManager, commandContextMenuDefinition));
            }
        }
        ...
        {
            var commandMenuBuilder = new CommandMenuBuilder(this.commandBar ? MenuType.CommandBarContext : MenuType.Context);
            ...
            this.MenuItems.AddRange(commandMenuBuilder.CreateMenuItems());
        }
        ...
        private Command ShowModal(Control parentWindow, Point position, int alternateXPosition)
        {
            ...
            this.RecursivelyRaiseBeforeShow(this.MenuItems);

            var rtl = BidiHelper.IsRightToLeft;

            var layout = TPM.LEFTALIGN;
            if (rtl)
            {
            ...
            var menuID = User32.TrackPopupMenu(this.Handle,
            ...
            var commandOwnerDrawMenuItem = this.LocateCommandOwnerDrawMenuItem(this.MenuItems, menuID);
            Debug.Assert(
                commandOwnerDrawMenuItem != null,
                "CommandContextMenu.ShowModal was not able to locate the CommandOwnerDrawMenuItem corresponding to the user's selection.");
        ...
        private void RecursivelyRaiseBeforeShow(MenuItemCollection menuItemCollection)
        {
            ...
            foreach (MenuItem menuItem in menuItemCollection)
            {
                ...
                if (menuItem is OwnerDrawMenuItem)
                {
                    ...
                    this.RecursivelyRaiseBeforeShow(menuItem.MenuItems);
                }
            }
        }
        ...
        private CommandOwnerDrawMenuItem LocateCommandOwnerDrawMenuItem(MenuItemCollection menuItemCollection, int menuID)
        {
            ...
            foreach (MenuItem menuItem in menuItemCollection)
            {
                ...
                {
                    var commandOwnerDrawMenuItem = (CommandOwnerDrawMenuItem)menuItem;
                ...
                {
                    var commandOwnerDrawMenuItem = this.LocateCommandOwnerDrawMenuItem(menuItem.MenuItems, menuID);
