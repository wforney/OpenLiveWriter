...
namespace OpenLiveWriter.Interop.Com
{
    ...
    public struct IEIFLAG
    {
        ...
        public const uint OFFLINE = 0x0008;      // if the extractor shouldn't hit the net to get any content needed for the rendering
        public const uint GLEAM = 0x0010;      // does the image have a gleam ? this will be returned if it does
        public const uint SCREEN = 0x0020;      // render as if for the screen  (this is exclusive with IEIFLAG_ASPECT )
        public const uint ORIGSIZE = 0x0040;      // render to the approx size passed, but crop if necessary
