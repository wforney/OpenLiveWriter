...
namespace OpenLiveWriter.SpellChecker
{
    ...
    /// <summary>
    /// The SortedMarkupRangeList class.
    /// </summary>
    public class SortedMarkupRangeList
    {
        ...
        public void Add(MarkupRange range)
        {
            ...
            try
            {
                this.words.Add(newRange.Start.PointerRaw, newRange);
            }
        ...
        /// </summary>
        /// <param name="testRange">The test range.</param>
        /// <returns><c>true</c> if [contains] [the specified test range]; otherwise, <c>false</c>.</returns>
        public bool Contains(MarkupRange testRange)
        {
            ...
            var pos = SortedMarkupRangeList.BinarySearch(this.words.Keys, testRange.Start.PointerRaw,
                                                         new MarkupPointerComparer());
            if (pos >= 0)
            {
                if (this.words.Values[pos].End.IsLeftOf(testRange.End))
                {
                    Debug.Fail("testRange partially overlaps with range");
                    try
                    {
                        Debug.WriteLine("testRange: [" + testRange.Text + "]");
                        Debug.WriteLine("thisRange: [" + this.words.Values[pos].Text + "]");
                    }
                    catch (Exception e)
                    {
            ...
            if (this.words.Values[pos].End.IsRightOf(testRange.Start))
            {
                if (this.words.Values[pos].End.IsLeftOf(testRange.End))
                {
#if DEBUG
                    var temp = testRange.Clone();
                    temp.Start.MoveToPointer(this.words.Values[pos].End);
                    if ((temp.Text ?? string.Empty).Trim().Length > 0)
                    {
                        Debug.Fail("testRange partially overlaps with range");
                        try
                        {
        ...
        public void ClearRange(MarkupRange clear)
        {
            ...
            var idx = SortedMarkupRangeList.BinarySearch(this.words.Keys, clear.Start.PointerRaw,
                                                         new MarkupPointerComparer());
            if (idx < 0)
            {
                idx = ~idx;
        ...
        /// </summary>
        /// <param name="p">The markup pointer.</param>
        ...
        public bool Contains(MarkupPointer p)
        {
            ...
            var idx = SortedMarkupRangeList.BinarySearch(this.words.Keys, p.PointerRaw, new MarkupPointerComparer());
            if (idx >= 0)
            {
                return true;
            }
            ...
            for (; idx >= 0; idx--)
            {
                var wordRange = this.words.Values[idx];
        ...
        /// bitwise complement of (the index of the last element plus 1).</returns>
        public static int BinarySearch<T>(IList<T> list, T value, IComparer<T> comparer)
        {
            ...
            while (min < max)
            {
                ...
                var result = comparer.Compare(value, list[mid]);
        ...
        /// </summary>
        public void Clear() => this.words.Clear();
    }
}
